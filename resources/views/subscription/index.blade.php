                ...
                <thead>
                    <thead>
                        <tr>
                            ...
                            <th class="py-2 px-4 text-left">Abonnement</th>
                    ...
                    <tbody class="pr-12">
                        @foreach ($plans as $plan)
                            <tr class="hover:shadow-lg transition-transform duration-300 ease-in-out">
                                <td class="py-2 px-4 text-left">{{ $plan->id }}</td>
                                <td class="py-2 px-4">{{ $plan->name }}</td>
                                @if($plan->interval = 'month')
                                <td class="py-2 px-4"   > Mois</td>
                                @endif
                                ...
                                <a href="{{  route('subscription-plans.edit', $plan->slug) }}"
                                    ...
                                    <button @click="open = true" class="bg-yellow-100 text-white py-2 px-4 rounded hover:bg-yellow-200">
                                        <i class="fa-solid fa-edit" style="color: #454700;"></i>
                                    </button>
                                </a>
                                <form action="{{ route('subscription-plans.destroy', $plan->id) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" class="text-red-500 hover:underline">
                                        <button @click="open = true" class="bg-red-100 text-white py-2 px-4 rounded hover:bg-red-300"
                                ...
                                @if ($plan->is_active)
                                    <form action="{{ route('subscription-plans.deactivate', $plan->id) }}" method="post">
                                        ...
                                        <button type="submit"><label
                                                class="relative inline-flex items-left cursor-pointer">
